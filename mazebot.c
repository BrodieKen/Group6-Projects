...
task main(){
	...
	//Setup Maze on Screen
	buildOuterBorders();
	MazeSim();
	refreshScreen();

	//Solve Maze Virtually
	...
	refreshScreen();
	optimizePath(); //Calculate Shortest Path
	populateDirectionMatrix();
	playSound(soundDownwardTones); //Play Sound When Solved
	...
	//sound a tone for 2 seconds, and indicate on the robot screen when it believes that it has reached the target coordinates.
...
//Rotate Clockwise 90degrees and update orientation
void turnRightIRL(){
...
//Rotate Counter-Clockwise 90degrees and update orientation
void turnLeftIRL(){
...
//Set Values for required amount of 90degree turns based on current orientation and desired orientation
void populateDirectionMatrix(){
	/*
	0	1	2	-1
	-1	0	1	2
...
//Construct Virtual Maze in Memory
void MazeSim(){
...
//Construct Outer Walls of Maze in Memory
void buildOuterBorders(){
...
//Update Display, Call every iteration
void refreshScreen(){
	eraseDisplay();
	gridDraw();
	drawBot();
...
//Draw Arrow on Screen to represent Robot's postition
void drawBot(){
