...
int targetRow = 4;
int targetCol = 8;
int robHeading = 1; // 0=North, 1=East, 2=South, 3=West
int robRow = startRow;
int robCol = startCol;
int orientation = 0;
int angle = 0;
...
task main(){
	...
	for (int i = 0; i < pathIndex; i++){
		robHeading = orientation;
		refreshScreen();
		if (pathTaken[i] == 9){
			break;
		}
		//go straight one grid length
		else if (pathTaken[i] == pathTaken[i+1]){
			goFwdIRL();
		}
		//the final node is ahead, turn to face it and go forward one grid length
		else if (pathTaken[i] < 4 && pathTaken[i+1] == 9){
		...
		//right turn ahead
		else if ((pathTaken[i] < pathTaken[i+1] && (pathTaken[i+1] != 3 && pathTaken[i] != 0)) || (pathTaken[i]== 3 && pathTaken[i+1] ==0)){
			int targetAngle = 0;
			switch(pathTaken[i]){
				case 0: targetAngle = 90;
				case 1: targetAngle = 0;
				case 2: targetAngle = -90;
				case 3: targetAngle = 180;
			}
			lineTrack(1, targetAngle);
		}
		//left turn ahead
		else if (pathTaken[i] > pathTaken[i+1] || (pathTaken[i]== 0 && pathTaken[i+1] ==3)){
			int targetAngle = 0;
			switch(pathTaken[i]){
				case 0: targetAngle = -90;
				case 1: targetAngle = 180;
				case 2: targetAngle = 90;
				case 3: targetAngle = 0;
			}
			lineTrack(-1, targetAngle);
		}
	}
...
void lineTrack(int allignmentVal, int targetAngle){
	...
	// switch(turnHeading){
	// 	case 0:
	// 	case 1: targetAngle = 80;
	// 	case 2: targetAngle = 170;
	// 	case 3: targetAngle = 80;
	// }
	if(allignmentVal ==1){
		setMotorTarget(armMotor,140,20);
	...
	if (targetAngle == 180){
		while((angle > 165 || angle < -165) == false){ //stop after ~90deg turn
			checkDirection();
			displayTextLine(1,"Angle: %d",angle);
			displayTextLine(2,"Target: %d",targetAngle);
			displayTextLine(3,"Orientation: %d",orientation);
			if(allignmentVal == 1){	//right track
				if(red < targetColorRed - 2 && green < targetColorGreen - 2){ //to black turn away from line
				//When Line is on right -> this is for left turns
				//When Line is on left -> this is for right turns
					setMotorSpeed(rightMotor, -0.5*motorSpeed*allignmentVal);
					setMotorSpeed(leftMotor, motorSpeed*allignmentVal);
					getColorRGB(colorSensor, red, green, blue);
				}
				else if(red > targetColorRed + 2 && green > targetColorGreen + 2){ //too white
				//When Line is on right -> this is for right turns
				//When Line is on left -> this is for left turns
					setMotorSpeed(rightMotor, motorSpeed*allignmentVal);
					setMotorSpeed(leftMotor, -0.5*motorSpeed*allignmentVal);
					getColorRGB(colorSensor, red, green, blue);
						}
				else if(red > targetColorRed - 2  && red < targetColorRed + 2 && green > targetColorGreen  - 2 && green < targetColorGreen + 2 ){ //on the edge
		...
		while(angle > targetAngle + 15 || angle < targetAngle - 15){ //stop after ~90deg turn
			checkDirection();
			displayTextLine(1,"Angle: %d",angle);
			displayTextLine(2,"Target: %d",targetAngle);
			displayTextLine(3,"Orientation: %d",orientation);
	...
	checkDirection();
		switch(orientation){
			case 0:
				if(allignmentVal == 1){
					setMotorSpeed(rightMotor, -5*allignmentVal);
					setMotorSpeed(leftMotor, 5*allignmentVal);
					waitUntil(getGyroHeading(gyroSensor) == 0);
					break;
				}
				else{
					setMotorSpeed(rightMotor, 5*allignmentVal);
			...
			case 2:
				if(allignmentVal == 1){
					setMotorSpeed(rightMotor, -5*allignmentVal);
					setMotorSpeed(leftMotor, 5*allignmentVal);
					waitUntil(getGyroHeading(gyroSensor) == 180);
					break;
				}
				else{
					setMotorSpeed(rightMotor, 5*allignmentVal);
	...
	checkDirection();
	switch(orientation){
		case 0: robCol -= allignmentVal; break;
		case 1: robRow += allignmentVal; break;
		case 2: robCol += allignmentVal; break;
		case 3: robRow -= allignmentVal; break;
		default: break;
	}
}
