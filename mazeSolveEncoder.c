...
int robCol = startCol;
int gridLength = 800;
int turnTarget = 150;	//found through experimentation rather than calulcation
int motorSpeed = 25;
int turnSpeed = 5;
...
void turnRightIRL(){
	resetMotorEncoder(leftMotor);
	resetMotorEncoder(rightMotor);
	setMotorTarget(leftMotor,turnTarget, turnSpeed);
	setMotorTarget(rightMotor, -turnTarget, turnSpeed);
	waitUntilMotorStop(leftMotor);
	waitUntilMotorStop(rightMotor);
	switch(robHeading){
