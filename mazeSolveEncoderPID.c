...
int robCol = startCol;
int gridLength = 300;
int turnTarget = 75;//150;	//found through experimentation rather than calulcation
float motorSpeed = 15;
int turnSpeed = 5;
...
void goFwdIRL(){
	...
	while ((getMotorEncoder(leftMotor) + getMotorEncoder(rightMotor))/2 < gridLength){
		...
		//set motor speeds
		setMotorSpeed(rightMotor, motorSpeed - pidDrive);
		setMotorSpeed(leftMotor, motorSpeed + pidDrive);
	}
	resetMotorEncoder(leftMotor);
	resetMotorEncoder(rightMotor);
	setMotorTarget(leftMotor,300,motorSpeed);
	setMotorTarget(rightMotor,300,motorSpeed);
...
void turnRightIRL(){
	...
	while((abs(getMotorEncoder(leftMotor))<turnTarget&&abs(getMotorEncoder(leftMotor))<turnTarget)||getColorReflected(colorSensor)>5){
